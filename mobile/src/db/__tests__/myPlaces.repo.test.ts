import { myPlacesRepo } from '../myPlaces.repo';
...
describe('myPlacesRepo', () => {
  ...
  it('upsert writes is_primary 0 when not primary', async () => {
    await myPlacesRepo.upsert({ ...sample, isPrimary: false }, 'u1');
    const [, ...args] = mockRunAsync.mock.calls[0]!;
    expect(args).toContain(0);
  });

  it('clearPrimaryExcept un-marks other primaries of the same type', async () => {
    await myPlacesRepo.clearPrimaryExcept('u1', 'WORKPLACE', 'a1');
    const [sql, ...args] = mockRunAsync.mock.calls[0]!;
    expect(sql).toMatch(/UPDATE my_places SET is_primary\s*=\s*0/);
    expect(sql).toMatch(/place_type\s*=\s*\?/);
  ...
  it('softDelete marks a single place is_deleted and is_dirty by id', async () => {
    await myPlacesRepo.softDelete('a1');
    const [sql, ...args] = mockRunAsync.mock.calls[0]!;
    expect(sql).toMatch(/UPDATE my_places SET is_deleted = 1, is_dirty = 1/);
    expect(args[args.length - 1]).toBe('a1');
  });

  it('listActive filters out soft-deleted rows', async () => {
    mockGetAllAsync.mockResolvedValueOnce([]);
    await myPlacesRepo.listActive('u1');
  ...
  it('listActive scopes by user, orders by created_at, and maps transport/isPrimary (stripping user_id)', async () => {
    ...
    const result = await myPlacesRepo.listActive('u1');
    ...
    expect(result[0]).toMatchObject({ id: 'a1', placeType: 'WORKPLACE', transport: 'TRANSIT', isPrimary: true });
    expect(result[1]).toMatchObject({ id: 'a2', placeType: 'OTHER', transport: 'CAR', isPrimary: false });
