package app.pickhouse.domain.photo.controller;
...
class PhotoControllerTest {
    ...
    void upload_returns_201_with_photo_dto() throws Exception {
        ...
        PhotoResponse dto = new PhotoResponse(photoId, null, "https://api/files/" + photoId + ".jpg",
            null, Instant.now());
        when(service.upload(eq(userId), eq(null), any(), eq(null), eq(null))).thenReturn(dto);
        ...
        mvc.perform(multipart("/photos/upload").file(file)
            ...
            .andExpect(jsonPath("$.data.remoteUrl").value("https://api/files/" + photoId + ".jpg"));
    }
    ...
    void upload_with_houseId_query_param_passes_to_service() throws Exception {
        ...
        UUID photoId = UUID.randomUUID();
        PhotoResponse dto = new PhotoResponse(photoId, houseId, "url", null, Instant.now());
        when(service.upload(eq(userId), eq(null), any(), eq(houseId), eq(null))).thenReturn(dto);
    ...
    void upload_with_id_param_passes_to_service() throws Exception {
        UUID userId = UUID.randomUUID();
        UUID photoId = UUID.randomUUID();
        PhotoResponse dto = new PhotoResponse(photoId, null, "url", null, Instant.now());
        when(service.upload(eq(userId), eq(photoId), any(), eq(null), eq(null))).thenReturn(dto);
