...
class HouseControllerTest {
    ...
    private static HouseResponse sampleDto(UUID id) {
        return new HouseResponse(id, null, DealType.JEONSE, 50000, 0,
    ...
    void create_returns_201_with_house_dto() throws Exception {
        ...
        mvc.perform(post("/houses").with(authentication(auth(userId)))
            ...
            .andExpect(jsonPath("$.data.id").value(houseId.toString()))
            .andExpect(jsonPath("$.data.dealType").value("JEONSE"));
    }
    ...
    void create_with_wizard_fields_round_trips_through_dto() throws Exception {
        UUID userId = UUID.randomUUID();
        UUID houseId = UUID.randomUUID();
        HouseResponse dto = new HouseResponse(houseId, null, DealType.WOLSE, 1000, 60,
            null, null, null, null, null, null, null,
            null, null, null, null, null,
            null, null, null,
            ...
            null,
            "우리집", null, null,
            app.pickhouse.domain.house.entity.RoomType.TWO_ROOM,
            app.pickhouse.domain.house.entity.FloorType.GROUND,
            app.pickhouse.domain.house.entity.Direction.SOUTH,
            List.of(app.pickhouse.domain.house.entity.MaintenanceUtility.WATER),
        ...
        mvc.perform(post("/houses").with(authentication(auth(userId)))
                .contentType(MediaType.APPLICATION_JSON).content(body))
            .andExpect(status().isCreated())
            .andExpect(jsonPath("$.data.nickname").value("우리집"))
            .andExpect(jsonPath("$.data.direction").value("SOUTH"))
            .andExpect(jsonPath("$.data.roomType").value("TWO_ROOM"))
            .andExpect(jsonPath("$.data.maintenanceIncludes[0]").value("WATER"))
            .andExpect(jsonPath("$.data.utilityEstimates.WATER").value(2))
            .andExpect(jsonPath("$.data.fullOption").value(true));
    }
    ...
    void photos_returns_house_photos_for_owner() throws Exception {
        ...
        PhotoResponse photo = new PhotoResponse(photoId, houseId,
        ...
        mvc.perform(get("/houses/" + houseId + "/photos").with(authentication(auth(userId))))
            ...
            .andExpect(jsonPath("$.data[0].id").value(photoId.toString()))
            .andExpect(jsonPath("$.data[0].remoteUrl").value("https://api/files/" + photoId + ".jpg"));
    }
